...
void PmergeMe::vMakePairs()
{
    ...
    for (size_t i = 0; i + 1 < _vector.size(); i += 2)
    {
        ...
        _vPairs.push_back((Pair){std::min(a, b), std::max(a, b), false});
    }
    if (_vector.size() % 2)
        _vPairs.push_back((Pair){_vector.back(), _vector.back(), true});
}
...
void PmergeMe::dMakePairs()
{
    ...
    for (size_t i = 0; i + 1 < _deque.size(); i += 2)
    {
        ...
        _dPairs.push_back((Pair){std::min(a, b), std::max(a, b), false});
    }
    if (_deque.size() % 2)
        _dPairs.push_back((Pair){_deque.back(), _deque.back(), true});
}
...
void PmergeMe::sort_vec(std::vector<unsigned int> &arr)
{
    ...
    for (size_t i = 0; i + 1 < arr.size(); i += 2)
        pairs.push_back((Pair){std::min(arr[i], arr[i + 1]), std::max(arr[i], arr[i + 1]), false});
    if (arr.size() % 2)
        pairs.push_back((Pair){arr.back(), arr.back(), true});
...
void PmergeMe::isSorted(unsigned int size)
{
    std::cout << "\033[33m" <<  size << " : " << _vector.size() << (size == (unsigned int)_vector.size() ?  "\033[32m Vector : As much values as original\033[0m\n" : "\033[31mVector : Not same amout of values\033[0m\n");
    std::cout << "\033[33m" << size << " : " << _deque.size() << (size == (unsigned int)_deque.size() ?  "\033[32m Deque : As much values as original\033[0m\n" : "\033[31mDeque : Not same amout of values\033[0m\n");
